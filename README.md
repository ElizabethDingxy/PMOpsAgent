...
- V2.3 高可靠协作与实时流式传输：Server-Sent Events (SSE) 实时分析追踪、Critic 自我反思审计循环、密集/稀疏双模向量长效知识库、TAPD 双向状态同步与增量同步
- 结构化 Agent 输出校验
...
DEEPSEEK_CRITIC_TIMEOUT_MS=60000
...
# 向量知识库配置（不配置时自动降级为本地高性能 TF-IDF 向量空间模型）
EMBEDDING_API_KEY=可选，OpenAI 或兼容格式的 Embedding API 密钥
...
如果你在飞书自定义机器人里开启了签名校验，请把机器人安全设置里生成的 Secret 粘贴到 `.env.local` 的 `FEISHU_BOT_SECRET`，并重启 `npm run dev`。发送时服务端会自动生成 `timestamp` 和 `sign`，不会把 Secret 暴露到前端。

如果你在飞书机器人里开启了关键词校验，飞书摘要中必须包含对应关键词，否则飞书会拒收。当前服务端发送 webhook 时会自动补上 `PMOpsAgent 评审摘要` 前缀，建议关键词配置为 `PMOpsAgent`。
...
## V2.3 流式分析、反思审计与双模向量库

为了将 PMOpsAgent 提升至生产级的可靠性与交互体验，我们在 V2.3 版本中引入了以下四大核心能力：

### 1. Server-Sent Events (SSE) 实时流式传输
- **重构流式 API**：`/api/analyze` 升级为 SSE 协议流，配合前端的异步流式 Reader，在执行期间向前端实时投递进度事件。
- **状态栏稳定排序**：前端设计了逻辑步骤权重评分与稳定排序算法，使分析期间各 Agent 步骤（包括反思重试）均能在垂直时间轴对应的槽位平滑转圈、打勾或报错，彻底杜绝了步骤乱序漂移。

### 2. Critic Agent 自动化自我反思与对比审计循环
- **多维度合规审计**：引入专门的 Critic Agent，对 PRD Agent 与 Delivery Agent 生成的产物进行质量审计，比对是否包含所有 MVP 必做项、是否排除越界功能、任务是否存在循环依赖、高风险项是否得到覆盖等。
- **流式驳回与修正**：如审计不通过，Critic 会输出详细修正意见，管线将自动携带意见退回 PRD 和 Delivery 阶段重新生成。自我反思修正支持最高 **3 次尝试**，如仍不通过则挂起等待人工确认。

### 3. 基于密集/稀疏双模向量的产品长效知识库
- **双模 Embedding 引擎**：支持 OpenAI 密集向量模式（`text-embedding-3-small`）与**本地无密钥 Fallback 稀疏向量模型（基于 TF-IDF 算法及中英文 Unigram/Bigram 分词器）**。这确保了在无网络/无 API 密钥情况下仍具有 100% 可用的高精语义检索能力。
- **增量缓存与检索**：仅对新增/有改动的产品要素提取向量，极大地节省了 Token 开销。历史运行的项目记忆可通过前台“向量 DB 看板”一键增量或全量重建，支持在飞书群聊中直接通过 @机器人 进行高精度语义模糊查询。

### 4. TAPD 双向状态同步与增量同步
- **状态双向读取**：支持一键向 TAPD 发送请求，拉取 Story 及关联 Task 的真实执行状态（未开始、进行中、已解决等）并持久化。
- **增量继承**：依托 Product Memory，在多轮运行中自动继承已同步的任务关系，自动标记为“已同步”并锁死，避免重复同步，支持增量追加。
...
DEEPSEEK_CRITIC_TIMEOUT_MS=90000
```
